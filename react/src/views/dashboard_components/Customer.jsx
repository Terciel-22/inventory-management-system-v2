...
export default function Customer(){
  ...
  const handleClick = (event) => {
    ...
    if (event.target.name === "delete")
    {
      ...
      {
        axiosClient.delete(`/customers/${customerIDRef.current.value}`)
        .then(()=>{
          setNotification("Successfully deleted customer.");
          resetForm();
        })
      }
      return;
    ...
    const customerFormData = new FormData(customerFormRef.current);
    if(regionRef.current.value === "NCR")
    {
      customerFormData.append("province","")
    }

    if(event.target.name === "add")
    {
      axiosClient.post(`/customers`,customerFormData)
      .then(()=>{
    ...
    {
      axiosClient.post(`/customers/${customerIDRef.current.value}?_method=PUT`,customerFormData)
      .then(()=>{
        setNotification("Successfully updated customer.");
        setErrors("");
      })
      .catch(error=>{
        const response = error.response;
  ...
  return (
    <form className="form-tab" id="customer" ref={customerFormRef}>
      {errors && 
      ...
      <div className="row">
        <div className="form-group">
          ...
          <input type="text" id="customer-full-name" name="full_name" ref={fullNameRef}/>
        </div>
        <div className="form-group">
          <label htmlFor="customer-status">Status</label>
          <select name="status" id="customer-status" ref={statusRef}>
        ...
        <div className="form-group">
          ...
          <input type="text" id="customer-id" name="id" className="id-selector" placeholder="Auto-generate on add" onChange={handleChange} ref={customerIDRef}/>
        </div>
      </div>
      <div className="row">
        <div className="form-group">
          <label htmlFor="customer-mobile">Mobile Number</label>
          <input type="text" id="customer-mobile" name="mobile" ref={mobileRef}/>
        </div>
        <div className="form-group">
          <label htmlFor="customer-telephone">Telephone Number</label>
          <input type="tel" id="customer-telephone" name="telephone" ref={telephoneRef}/>
        </div>
        <div className="form-group">
          <label htmlFor="customer-email">Email Address</label>
          <input type="email" id="customer-email" name="email" ref={emailRef}/>
        </div>
      </div>
      <div className="row">
        <div className="form-group">
          <label htmlFor="customer-address">Address</label>
          <input type="text" id="customer-address" name="address" ref={addressRef}/>
        </div>
      </div>
      <div className="row">
        <div className="form-group">
          <label htmlFor="customer-region">Region</label>
          <select name="region" defaultValue="" id="customer-region" ref={regionRef} onChange={handleChange}>
            <option value="" hidden>--Select Region--</option>
          </select>
        </div>
        <div className="form-group">
          <label htmlFor="customer-province">Province</label>
          <select name="province" defaultValue="" id="customer-province" ref={provinceRef} onChange={handleChange}>
            <option value="" hidden>--Select Province--</option>
          </select>
        </div>
        <div className="form-group">
          <label htmlFor="customer-city-municipality">City/Municipality</label>
          <select name="city_municipality" defaultValue="" id="customer-city-municipality" ref={cityMunicipalityRef} onChange={handleChange}>
            <option value="" hidden>--Select City/Municipality--</option>
          </select>
        </div>
        <div className="form-group">
          <label htmlFor="customer-barangay">Barangay</label>
          <select name="barangay" defaultValue="" id="customer-barangay" ref={barangayRef}>
